...
export default function RegisterPage() {
  ...
  return (
    <motion.div 
      ...
      className="min-h-screen flex flex-row-reverse w-full overflow-hidden"
    >
      
      {/* Right Split - Image / Branding (Reversed for Register) */}
      <div className="hidden lg:flex w-1/2 relative bg-black items-center justify-center overflow-hidden">
        <div 
          className="absolute inset-0 bg-cover bg-center opacity-80" 
          style={{ backgroundImage: 'url(/images/signup_split_1781676374813.png)' }}
        ></div>
        <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-transparent to-black/80"></div>
        
        {/* Subtle decorative SVGs */}
        <svg className="absolute bottom-10 left-10 w-32 h-32 text-orange-500 opacity-30" viewBox="0 0 100 100" fill="currentColor">
          <polygon points="50,10 90,90 10,90" />
        </svg>
        <svg className="absolute top-20 right-20 w-24 h-24 text-red-500 opacity-20" viewBox="0 0 100 100" fill="currentColor">
          <circle cx="50" cy="50" r="40" />
        </svg>

        <div className="relative z-10 p-12 max-w-xl text-center">
          <h2 className="text-5xl font-bold text-white tracking-widest uppercase drop-shadow-2xl mb-6">Join Us</h2>
          <p className="text-xl text-orange-200 font-light italic drop-shadow-md">
            "Every big financial goal begins with a single step. Create your profile and start tracking today."
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 lg:p-24 relative bg-[#f8fafc]">
        ...
        <div className="absolute top-0 left-0 w-64 h-64 bg-red-100 rounded-full blur-3xl opacity-50 -ml-20 -mt-20"></div>
        <div className="absolute bottom-0 right-0 w-64 h-64 bg-orange-100 rounded-full blur-3xl opacity-50 -mr-20 -mb-20"></div>
        ...
        <div className="w-full max-w-md relative z-10 animate-scale-in">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              ...
              {errors.name && (
                <p className="mt-1 text-xs text-red-500 font-sans">{errors.name.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full rounded-xl border bg-white px-4 py-3 pl-12 text-lg text-[#2D1B15] placeholder-slate-300 outline-none transition-all font-sans shadow-sm
                    focus:ring-2 focus:ring-orange-300
                    ${errors.confirmPassword ? 'border-red-400' : 'border-slate-200 focus:border-orange-400'}`}
                />
              </div>
              {errors.confirmPassword && (
                <p className="mt-1 text-xs text-red-500 font-sans">{errors.confirmPassword.message}</p>
              )}
            </div>
